...
int monitor_devices(pid_t ppid){
  ...
  for (int i = 0; i < NUM_PROCESS; i++) {
    ...
    /*
     *  The thing is, for parallel we will have to monitor multple subsystems.
     *  [parallel, printer, lp]
     */
    if (i == 2) {
...
int monitor_avahi_devices(pid_t parent_pid) {
  ...
  if (!(sb = avahi_service_browser_new(client, AVAHI_IF_UNSPEC,
				       AVAHI_PROTO_UNSPEC, "_ipp-tls._tcp",
				       NULL, 0, browse_callback, client))) {
    fprintf (stderr, "Failed to create service browser: %s\n",
	     avahi_strerror(avahi_client_errno(client)));
    goto fail;
  ...
  if (!(sb = avahi_service_browser_new(client,AVAHI_IF_UNSPEC,
				       AVAHI_PROTO_UNSPEC, "_printer._tcp",
				       NULL, 0, browse_callback, client))) {
    fprintf(stderr, "Failed to create service browser: %s\n",
	    avahi_strerror(avahi_client_errno(client)));
    goto fail;
